...
var mqantlib=function(mqtt){
    var mqant = cc.Class({
        ...
        request:function(topic,msg,callback,callbackContext){
            if(!this.connected()) return;   //若server已經斷開，則需要針對是否已經連線做判斷
        ...
        parseUTF8:function(payload){
            ...
            var byteArray = new Uint8Array(payload);
            var messages = unzip(byteArray);
            if (typeof messages === "string")
				return messages;
			else
				return mqtt.ParseUTF8(messages, 0, messages.length);
        }
    })
    return  mqant
};
