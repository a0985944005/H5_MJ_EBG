...
//    	    	break;
	...
	ClientImpl.prototype._doConnect = function (wsurl) {
		...
		//加入不帶port判斷
		let _NewWSURL 	= wsurl.split(":");
		let _CheckChar 	= _NewWSURL[2].split("/");
		if(parseInt(_CheckChar) == 0){
			_NewWSURL[2] = _CheckChar[1];
			wsurl = _NewWSURL[0] + ":" + _NewWSURL[1] + "/" + _NewWSURL[2];
			this._wsuri = wsurl;
		}
   ...
   * up to 128 seconds.
	...
	var Client = function (host, port, path, clientId) {
	    ...
	    if (arguments.length == 2) {
			...
			let _WSS = cc.module.jsonFile["SERBER_WSS"] == true ? "wss" : "ws";
			uri = _WSS+"://"+(ipv6AddSBracket?"["+host+"]":host)+":"+port+path;
		}
			...
			if (connectOptions.hosts) {
				...
				if (!usingURIs) {
					...
					for (var i = 0; i<connectOptions.hosts.length; i++) {
						...
						let _WSS = cc.module.jsonFile["SERBER_WSS"] == true ? "wss" : "ws";
						uri = _WSS+"://"+(ipv6?"["+host+"]":host)+":"+port+path;
